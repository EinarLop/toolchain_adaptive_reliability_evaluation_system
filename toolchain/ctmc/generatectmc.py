...
def save_ctmc_drawing(ctmc_graph, filename, labels=None, graph_layout='shell',
    ...
    nx.draw_networkx_nodes(ctmc_graph, graph_pos, node_size=node_size, alpha=node_alpha, node_color=node_color, ax=axis)
    nx.draw_networkx_edges(ctmc_graph, graph_pos, width=edge_tickness, alpha=edge_alpha, edge_color=edge_color, arrows=True, ax=axis)

    # Node labels
    states = []
    for node in ctmc_graph.nodes():
    ...
    for state_graph in ctmc_graph.nodes():
        ...
        state_name = str(state_graph.nodes())
        state_idx = states.index(state_name)
        state_labels[state_graph] = str(state_idx) + "\n" + textwrap.fill(state_name, 10)

    nx.draw_networkx_labels(
        ctmc_graph, graph_pos, state_labels,
    ...
    # Edge labels
    edge_labels = {}
    for e in ctmc_graph.edges():
        edge_labels[e] = ctmc_graph[e[0]][e[1]]['failed_element']

    nx.draw_networkx_edge_labels(
        ctmc_graph, graph_pos, edge_labels,
        font_size=node_text_size, font_family=text_font,
        ax=axis
    )
...
def save_graph_drawing(graph, filename, labels=None, graph_layout='spring',
    ...
    for vertex in graph.nodes():
        try:
            colors.append(graph.nodes[vertex]['color'])
        except KeyError:
            colors.append('white')
    ...
    for u, v in graph.edges():
        if graph[u][v]['coverage_vector'] == 0:
...
def add_rate(ctmc, src_state, dst_state, failed_element):
    if ctmc.has_edge(src_state, dst_state):
        ctmc[src_state][dst_state]['failed_element'].append(failed_element)
    else:
...
def explore(G, F, ctmc, is_correct, *args):
    for v in G.nodes():
        ...
        #cc_subgraphs = nx.connected_component_subgraphs(nx.Graph(H), copy=False)
        cc_subgraphs = (nx.Graph(H).subgraph(c) for c in nx.connected_components(nx.Graph(H)))
        for cc in list(cc_subgraphs):
            if not is_correct(cc, *args):
                for cc_vertex in cc.nodes():
        ...
        for state in ctmc.nodes():
            if nx.is_isomorphic(nx.Graph(state), nx.Graph(H), node_match=colors_match):
...
def generate_ctmc(G, ctmc, is_correct, *args):
    ...
    for vertex in G.nodes():
        for equivalence_class, class_color in class_to_color.items():
            if vertex in equivalence_class:
                G.nodes[vertex]['color'] = class_color
